...
function RoomLayout() {
  ...
  useEffect(() => {
    ...
    ref.current.peer = new Peer(undefined, {
      host: 'rzth-peerjs.herokuapp.com',
      port: '443',
      secure: true
    });
    ...
    peer.on("call", (call) => {
      ...
      call.on("stream", (recepientStream) => {
        ...
        socket.emit("set-info", ref.current.userId, call.peer, username, { video: null, audio: ref.current.stream.getAudioTracks()[0].enabled });
      });
    });
  ...
  const connectToNewUser = (userId, name, stream, peer, socket) => {
    ...
    call.on("stream", (recepientStream) => {
      ...
      socket.emit("set-info", ref.current.userId, userId, username, { video: null, audio: stream.getAudioTracks()[0].enabled });
    });
  }
